...
class KDTree
{
  ...
  private:
    ...
    KDTreeNode* constructorHelper(vector<Point<Dim>>& newPoints, int left, int right, int dim);
    int partition(vector<Point<Dim>>& vect, int left, int right, int pivot, int dim);
    Point<Dim> select(vector<Point<Dim>>& vect, int left, int right, int k, int dim);
    void swap(vector<Point<Dim>>& vect, int one, int two);

    // findNearestNeighbor helper function
    KDTreeNode* findRecursive(const Point<Dim>& query, KDTreeNode* node, int d) const;
};
